...
def main():
    parser = argparse.ArgumentParser(
        description="Extract all functions from a C project."
    )
    parser.add_argument(
        "project_dir",
        nargs="?",
        default="ATP_CODE/INIT",
        help="Root directory of the project",
    )
    parser.add_argument(
        "--types-json",
        default=".analysis/INIT_global_types.json",
        help="Path to global_types.json",
    )
    parser.add_argument(
        "--globals-json",
        default=".analysis/global_variables.json",
        help="Path to global_variables.json",
    )
    parser.add_argument(
        "--output",
        "-o",
        default=".analysis/INIT_functions.json",
        help="Output JSON file path",
    )
    args = parser.parse_args()
    refresh_functions(
        args.project_dir, args.types_json, args.globals_json, args.output
    )
